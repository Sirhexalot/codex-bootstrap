...
Write-Host "Running workstation setup for Codex on Windows."
Write-Host "This script may install global tools and write user-level Codex config under ~/.codex."
Write-Host "For customer-project initialization, use .\scripts\init-project.ps1 instead."
Write-Host ""
Write-Host "Setting up Python 3, Node.js/npm, pnpm, Playwright, FFmpeg, ImageMagick, Ghostscript, Remotion tooling, Git Bash, Composio, Meta Ads CLI, MarkItDown MCP, Teams MCP, and Codex skills on Windows..."
...
function Add-TeamsMcpConfig {
  ...
  $hasTeamsMcp = Select-String -Path $codexConfig -Pattern '^\[mcp_servers\.teams_mcp\]' -Quiet
