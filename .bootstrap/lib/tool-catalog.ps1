  ...
  switch ($Name) {
    ...
    "documents" {
      if ($Mode -eq "global") {
        ...
        Write-ToolMetadata -Name $Name -Mode $Mode -ScopeSupport "global_or_project" -TargetDir $script:GlobalPythonVenv -Commands "codex-python,codex-markitdown,pandoc" -Packages "winget:pandoc|python:openpyxl,python-docx,python-pptx,markitdown|npm:mammoth,docx,xlsx,pptxgenjs,pdf-parse" -Notes "Global document workbench for Office generation and Office/PDF extraction, including Node document parsers and generators."
