...
echo "Running workstation setup for Codex on macOS."
echo "This script may install global tools and write user-level Codex config under ~/.codex."
echo "For customer-project initialization, use ./scripts/init-project.sh instead."
echo
echo "Setting up Python 3, Node.js/npm, pnpm, Playwright, FFmpeg, Remotion tooling, Composio, Meta Ads CLI, Teams MCP, macOS MCP servers, and Codex skills on macOS..."
...
configure_teams_mcp() {
  ...
  if ! grep -q '^\[mcp_servers\.teams_mcp\]' "$codex_config"; then
    cat >> "$codex_config" <<'EOF'
